...
const fallbackHotActivities: HotActivity[] = [
  {
    id: 1,
    title: '五常有机新米',
...
export default function PointsMall({ onClose, requireAuth, balance, onBalanceChange }: Props) {
  ...
  const [hotActivities, setHotActivities] = useState<HotActivity[]>(fallbackHotActivities);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      try {
        const mallActs = await api.mallActivities();
        setHotActivities((mallActs.list || []).length ? (mallActs.list as HotActivity[]) : fallbackHotActivities);
        trackCEvent('c_mall_activities_load_success', { total: Number((mallActs.list || []).length) });
      } catch {
        setHotActivities(fallbackHotActivities);
  ...
  const products = useMemo(
    () =>
      items.map((item, idx) => ({
        ...item,
        image: resolveMediaUrl(item.image) || fallbackProductImages[idx % fallbackProductImages.length],
  ...
  const hotActivityCards = useMemo(
    () =>
      hotActivities.map((activity, idx) => ({
        ...activity,
        image: resolveMediaUrl(activity.image) || fallbackHotActivities[idx % fallbackHotActivities.length]?.image || '',
      })),
    [hotActivities]
  );
    ...
    >
      ...
      <AnimatePresence>
        {selectedProduct && (
          ...
          >
            <header className="sticky top-0 z-10 bg-white border-b border-slate-200 px-4 py-4 flex items-center justify-between">
              <button onClick={() => setSelectedProduct(null)} className="text-[#13a4ec] p-1">
                <ChevronLeft size={28} />
              </button>
            ...
            <main className="flex-1 overflow-y-auto p-4 space-y-4">
              <div className="bg-white rounded-xl overflow-hidden border border-slate-100">
                ...
                <div className="p-4">
                  ...
                  <p className="mt-2 text-sm text-slate-600">{selectedProduct.description || '暂无商品描述'}</p>
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <div className="rounded-lg bg-sky-50 p-3">
                      <p className="text-xs text-slate-500">所需积分</p>
                      <p className="text-xl font-bold text-[#13a4ec]">{selectedProduct.pointsCost}</p>
                    </div>
                    <div className="rounded-lg bg-emerald-50 p-3">
                      <p className="text-xs text-slate-500">库存</p>
                      <p className="text-xl font-bold text-emerald-600">{selectedProduct.stock}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <AnimatePresence>
        {selectedHotActivity && (
          ...
          >
            <header className="sticky top-0 z-10 bg-white border-b border-slate-200 px-4 py-4 flex items-center justify-between">
              <button onClick={() => setSelectedHotActivity(null)} className="text-[#13a4ec] p-1">
                <ChevronLeft size={28} />
              </button>
            ...
            <footer className="fixed bottom-0 left-0 right-0 z-10 bg-white border-t border-slate-200 p-4">
              <button
                ...
                disabled={joiningActivityId === Number(selectedHotActivity.id)}
                className="w-full rounded-xl py-3 text-sm font-bold text-white bg-[#13a4ec] disabled:bg-sky-200"
              >
                {joiningActivityId === Number(selectedHotActivity.id) ? '参与中...' : '立即参与'}
              </button>
            </footer>
          </motion.div>
        )}
